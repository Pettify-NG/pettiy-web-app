...
export default function PayoutTable ({
   ...
    const [lazyPayouts, setLazyPayouts] = useState<IListing[] | null>(null); 
...
        console.log(event); 
     }; 
    ...
    const matchedPayouts = useMemo(() => {
      if (searchValue.trim().length === 0) return getPayoutsByDate;
  
      return getPayoutsByDate?.filter(
        (payout) =>
          payout.breed.toLowerCase().includes(searchValue) ||
          payout.description.toLowerCase().includes(searchValue)
      );
    }, [getPayoutsByDate, searchValue]);
